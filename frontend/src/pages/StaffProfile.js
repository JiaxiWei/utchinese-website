...
import { getStaffProfile, saveStaffProfile, uploadStaffAvatar } from '../utils/api';
            ...
            <CardHeader>
              ...
              <div className="identity">
                <div className="name-section">
                  <h2>
                    {i18n.language === 'zh' 
                      ? (profile?.name_zh || user?.username) 
                      : (profile?.name_en || user?.username)
                    }
                    <MdVerified 
                      className={`verification-icon ${profile?.status === 'approved' ? 'verified' : 'unverified'}`}
                      title={profile?.status === 'approved' ? t('staff.profile.verified') : t('staff.profile.unverified')}
                    />
                  </h2>
                  {profile?.name_zh && profile?.name_en && (
                    <span className="alt-name">
                      {i18n.language === 'zh' ? profile.name_en : profile.name_zh}
                    </span>
                  )}
                </div>
                ...
                <div className="position-section">
                  <p className="position">
                    {i18n.language === 'zh' 
                      ? (profile?.position_zh || t('staff.profile.permissions.defaultPosition'))
                      : (profile?.position_en || t('staff.profile.permissions.defaultPosition'))
                    }
                  </p>
                  ...
                  {profile?.department && (
                    <div className="department-tag">
                      ...
                      <span>
                        {profile.department === 'ARTS & CULTURE GROUP' && t('staff.profile.departments.arts')}
                        {profile.department === 'CAREER & ACADEMIC GROUP' && t('staff.profile.departments.career')}
                        {profile.department === 'OPERATION GROUP' && t('staff.profile.departments.operation')}
                        {profile.department === 'SUPPORT GROUP' && t('staff.profile.departments.support')}
                      </span>
                    </div>
                  )}
                </div>
                ...
                <PersonalInfo>
                  {(i18n.language === 'zh' ? profile?.bio_zh : profile?.bio_en) ? (
                    <p className="bio">{i18n.language === 'zh' ? profile.bio_zh : profile.bio_en}</p>
                  ) : (
